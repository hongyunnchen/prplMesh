                                        ...
                                        task_pool &tasks)
{
    ...
    case beerocks_message::ACTION_BML_CHANNEL_SCAN_SET_CONTINUOUS_ENABLE_REQUEST: {
        ...
        auto request =
            beerocks_header->addClass<beerocks_message::cACTION_BML_CHANNEL_SCAN_SET_CONTINUOUS_ENABLE_REQUEST>();
        if (!request) {
    ...
    case beerocks_message::ACTION_BML_DCS_SET_CONTINUOUS_SCAN_ENABLE_REQUEST: {
        LOG(TRACE) << "ACTION_BML_DCS_SET_CONTINUOUS_SCAN_ENABLE_REQUEST";

        auto request = beerocks_header->addClass<beerocks_message::cACTION_BML_DCS_SET_CONTINUOUS_SCAN_ENABLE_REQUEST>();
        if (!request) {
            LOG(ERROR) << "addClass cACTION_BML_DCS_SET_CONTINUOUS_SCAN_ENABLE_REQUEST failed";
            break;
        }

        auto response = message_com::create_vs_message<
            beerocks_message::cACTION_BML_DCS_SET_CONTINUOUS_SCAN_ENABLE_RESPONSE>(
            cmdu_tx, beerocks_header->id());
        if (!response) {
            LOG(ERROR)
                << "Failed building message cACTION_BML_DCS_SET_CONTINUOUS_SCAN_ENABLE_RESPONSE !";
            return;
        }
        ...
        LOG(DEBUG) << "request radio_mac:" << radio_mac;
        bool success              = database.set_dcs_is_enabled(radio_mac, enable);
        response->op_error_code() = (success) ? eDcsOpErrCode::DCS_OP_NO_ERROR
                                              : eDcsOpErrCode::DCS_OP_INVALID_PARAMS_ENABLE;
    ...
    case beerocks_message::ACTION_BML_DCS_GET_CONTINUOUS_SCAN_ENABLE_REQUEST: {
        LOG(TRACE) << "ACTION_BML_DCS_GET_CONTINUOUS_SCAN_ENABLE_REQUEST";

        auto request = beerocks_header->addClass<beerocks_message::cACTION_BML_DCS_GET_CONTINUOUS_SCAN_ENABLE_REQUEST>();
        if (!request) {
            LOG(ERROR) << "addClass cACTION_BML_DCS_GET_CONTINUOUS_SCAN_ENABLE_REQUEST failed";
            break;
        }

        auto response = message_com::create_vs_message<
            beerocks_message::cACTION_BML_DCS_GET_CONTINUOUS_SCAN_ENABLE_RESPONSE>(
            cmdu_tx, beerocks_header->id());
        if (!response) {
            LOG(ERROR)
                << "Failed building message cACTION_BML_DCS_GET_CONTINUOUS_SCAN_ENABLE_RESPONSE !";
            return;
        }

        auto radio_mac = network_utils::mac_to_string(request->radio_mac());
        LOG(DEBUG) << "request radio_mac:" << radio_mac;
        response->isEnable() = (database.get_dcs_is_enabled(radio_mac)) ? 1 : 0;

        //send response to bml
        message_com::send_cmdu(sd, cmdu_tx);
        break;
    }
    case beerocks_message::ACTION_BML_DCS_GET_SCAN_RESULTS_REQUEST: {
        LOG(TRACE) << "ACTION_BML_DCS_GET_SCAN_RESULTS_REQUEST";

        auto request =
            beerocks_header->addClass<beerocks_message::cACTION_BML_DCS_GET_SCAN_RESULTS_REQUEST>();
        if (!request) {
            LOG(ERROR) << "addClass cACTION_BML_DCS_GET_SCAN_RESULTS_REQUEST failed";
            break;
        }
        ...
        uint8_t result_status = eDcsScanErrCode::DCS_SCAN_NO_ERROR;
        uint8_t op_error_code = eDcsOpErrCode::DCS_OP_NO_ERROR;
        uint8_t last          = 0;

        auto scan_in_progress = database.get_dcs_scan_in_progress(radio_mac, is_single_scan);
        if (scan_in_progress) {
            LOG(ERROR) << "Scan is still running!";
            op_error_code = eDcsOpErrCode::DCS_OP_SCAN_IN_PROGRESS;
        }

        auto last_scan_success = database.get_dcs_last_scan_success(radio_mac, is_single_scan);
        if (last_scan_success != eDcsScanErrCode::DCS_SCAN_NO_ERROR) {
            LOG(ERROR) << "Last scan did not finish successfully!";
            result_status = last_scan_success;
        }

        auto scan_results      = database.get_dcs_scan_results(radio_mac, is_single_scan);
        ...
        const size_t result_element_size = sizeof(beerocks_message::sDcsChannelScanResults);
        ...
        if (scan_results_size == 0) {
            ...
            //op_error_code = eDcsOpErrCode::DCS_OP_SCAN_RESULTS_EMPTY;
        }

        auto response = message_com::create_vs_message<
            beerocks_message::cACTION_BML_DCS_GET_SCAN_RESULTS_RESPONSE>(cmdu_tx,
                                                                         beerocks_header->id());

        if (op_error_code != eDcsOpErrCode::DCS_OP_NO_ERROR ||
            result_status != eDcsScanErrCode::DCS_SCAN_NO_ERROR) {
        ...
        beerocks_message::sDcsChannelScanResults *results = nullptr;
        int i;

        for (auto dump : scan_results) {

            if (results == nullptr) {
                LOG(DEBUG) << "Creating new ACTION_BML_DCS_GET_SCAN_RESULTS_RESPONSE !";
                response = message_com::create_vs_message<
                    beerocks_message::cACTION_BML_DCS_GET_SCAN_RESULTS_RESPONSE>(
                    cmdu_tx, beerocks_header->id());
                if (!response) {
                    LOG(ERROR)
                        << "Failed building message cACTION_BML_DCS_GET_SCAN_RESULTS_RESPONSE !";
                    return;
                }

                //reset message flags
                result_status = eDcsScanErrCode::DCS_SCAN_NO_ERROR;
                op_error_code = eDcsOpErrCode::DCS_OP_NO_ERROR;
                last          = 0;
                ...
                if (response->alloc_results(cmdu_max_result_count) == false) {
                    ...
                    op_error_code = eDcsOpErrCode::DCS_OP_ERROR;
                    break;
                }
    ...
    case beerocks_message::ACTION_BML_DCS_START_SCAN_REQUEST: {
        LOG(TRACE) << "ACTION_BML_DCS_START_SCAN_REQUEST";

        auto request = beerocks_header->addClass<beerocks_message::cACTION_BML_DCS_START_SCAN_REQUEST>();
        if (!request) {
            LOG(ERROR) << "addClass cACTION_BML_DCS_SET_CONTINUOUS_SCAN_ENABLE_REQUEST failed";
            break;
        }
        ...
        auto response =
            message_com::create_vs_message<beerocks_message::cACTION_BML_DCS_START_SCAN_RESPONSE>(
                cmdu_tx, beerocks_header->id());
        if (!response) {
            LOG(ERROR)
                << "Failed building message cACTION_BML_DCS_SET_CONTINUOUS_SCAN_ENABLE_RESPONSE !";
        ...
        LOG(DEBUG) << "set_dcs_dwell_time_msec " << dwell_time_ms;
        if (!database.set_dcs_dwell_time_msec(radio_mac, dwell_time_ms, true)) {
            LOG(ERROR) << "set_dcs_dwell_time_msec failed";
            response->op_error_code() = eDcsOpErrCode::DCS_OP_INVALID_PARAMS_DWELLTIME;
            message_com::send_cmdu(sd, cmdu_tx);
            break;
        }

        LOG(DEBUG) << "set_dcs_channel_pool " << channel_pool_size;
        if (!database.set_dcs_channel_pool(radio_mac, channel_pool_set, true)) {
            LOG(ERROR) << "set_dcs_channel_pool failed";
            response->op_error_code() = eDcsOpErrCode::DCS_OP_INVALID_PARAMS_CHANNELPOOL;
            message_com::send_cmdu(sd, cmdu_tx);
            break;
        }

        LOG(DEBUG) << "Clearing DB results for a single scan";
        if (!database.clear_dcs_scan_results(radio_mac, true)) {
            LOG(ERROR) << "failed to clear scan results";
            response->op_error_code() = eDcsOpErrCode::DCS_OP_ERROR;
            message_com::send_cmdu(sd, cmdu_tx);
            break;
        }
        ...
        tasks.push_event(database.get_dynamic_channel_selection_task_id(radio_mac),
                         ...
                         (void *)&new_event);
        response->op_error_code() = eDcsOpErrCode::DCS_OP_NO_ERROR;
        message_com::send_cmdu(sd, cmdu_tx);
        break;
    }
    case beerocks_message::ACTION_BML_DCS_DUMP_SCAN_RESULTS_REQUEST: {
        LOG(TRACE) << "ACTION_BML_DCS_DUMP_SCAN_RESULTS_REQUEST";
        break;
    }
    default: {
